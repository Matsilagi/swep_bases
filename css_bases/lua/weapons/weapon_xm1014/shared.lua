...
if ( SERVER ) then
	...
	if ( !util.IsValidModel( "models/weapons/v_shot_xm1014.mdl" ) ) then
...
if ( CLIENT ) then
	...
	SWEP.PrintName			= "#Cstrike_WPNHUD_xm1014"
	...
	killicon.AddFont( SWEP.ClassName, "CSKillIcons", "B", Color( 255, 80, 0, 255 ) )
...
function SWEP:PrimaryAttack()
	...
	self.Weapon:SetNextPrimaryFire( CurTime() + self.Weapon:SequenceDuration() - self.CycleTime );
	self.Weapon:SetNextSecondaryFire( CurTime() + self.Weapon:SequenceDuration() - self.CycleTime );
	self.m_flNextPrimaryAttack = CurTime() + self.Weapon:SequenceDuration() - self.CycleTime;
