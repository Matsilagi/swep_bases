...
function SWEP:DrawHUD()
	...
	if not self.xhair.colorstr then
		if self.xhair.colorstr then
			...
			self.xhair.color = Color(0,255,0,0)
			self.xhair.colorstr = "0 255 0 0"
			CreateClientConVar("cl_crosshair_color","0 255 0 0",true,true)
		end
		self.xhair.color = string.Explode(" ", self.xhair.colorstr)
		self.xhair.color = Color(self.xhair.color[1], self.xhair.color[2], self.xhair.color[3], self.xhair.color[4])
	end
	if self.Owner:GetFOV() ~= 75 and self.data.scope then
		//Hide the viewmodel
		local fov = 10 + (75 - self.Owner:GetFOV())
